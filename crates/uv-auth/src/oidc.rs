...
///
/// Many providers (e.g., Microsoft Entra) require a pre-registered client ID and will reject
/// this default. Configure [`OidcConfig::client_id`] or pass `--client-id` for those servers.
...
///
/// Generates a 32-byte cryptographically random code verifier (base64url-encoded) and
/// computes the S256 code challenge (SHA256 hash of the verifier, base64url-encoded),
/// per RFC 7636 §4.1.
...
/// caller to fall back to other authentication methods. The fall-through cause is
/// surfaced at `warn!` level so users can see why discovery was skipped without
/// `RUST_LOG=debug`.
pub async fn discover(
    ...
    let response = match client.get(discovery_url.clone()).send().await {
        ...
        Err(err) => {
            warn!("OIDC discovery request to {discovery_url} failed: {err}");
            return Ok(None);
        }
    };
    ...
    let document: OidcDiscoveryDocument = match response.json().await {
        Ok(doc) => doc,
        Err(err) => {
            warn!("Failed to parse OIDC discovery document at {discovery_url}: {err}");
            return Ok(None);
        }
    };
...
///
/// Sends a POST to the `device_authorization_endpoint` from
/// [`OidcDiscoveryDocument`] with the [`PkceChallenge`] and returns the
...
/// conditions per the RFC. Stops with [`OidcError::TokenExpired`] when the
/// device-code lifetime advertised by the server (`expires_in`) elapses, even if
/// the server keeps responding `authorization_pending`.
///
/// If `client_id` is `None`, defaults to `"uv"`.
pub async fn poll_for_token(
    ...
    debug!(
        "Polling token endpoint at {endpoint} with interval {interval}s (expires in {}s)",
...
mod tests {
    ...
    /// End-to-end test of the device authorization flow against a mock server:
    /// discovery succeeds, device authorization succeeds, polling returns
    /// `authorization_pending` once and then the access token.
    #[tokio::test]
    async fn test_device_flow_end_to_end() {
        ...
        let discovery = discover(&client, &base_url)
            .await
            .expect("discovery request")
            .expect("discovery document present");

        let pkce = generate_pkce();
        let device_response = device_authorize(&client, &base_url, &discovery, &pkce, None, None)
            .await
            .expect("device authorize");
        assert_eq!(device_response.user_code, "USER-CODE");
    ...
    /// Verify that polling stops with [`OidcError::TokenExpired`] when the device-code
    /// lifetime elapses, even if the server keeps returning `authorization_pending`.
    #[tokio::test]
    async fn test_poll_for_token_respects_expires_in() {
        use serde_json::json;
        use wiremock::matchers::{method, path};
        use wiremock::{Mock, MockServer, ResponseTemplate};
        ...
        let discovery = OidcDiscoveryDocument {
            issuer: server.uri(),
            device_authorization_endpoint: "/device/authorize".into(),
        ...
        .await;
        assert!(
            matches!(result, Err(OidcError::TokenExpired)),
            "got {result:?}"
        );
    }

    /// `discover` returns `Ok(None)` for a server that does not expose
    /// `.well-known/openid-configuration`, allowing the caller to fall through
    /// to other authentication strategies.
    #[tokio::test]
    async fn test_discover_returns_none_on_404() {
        use wiremock::matchers::{method, path};
        use wiremock::{Mock, MockServer, ResponseTemplate};
        ...
        let base_url = Url::parse(&server.uri()).unwrap();
        let client = reqwest::Client::new();
        let result = discover(&client, &base_url).await.expect("discover");
